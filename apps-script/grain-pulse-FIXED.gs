...
function updateSupabase(records, emailDate, snapshotType) {
  ...
  for (const record of records) {
    ...
    if (record.symbol.includes('|')) {
      console.log(`Skipping options symbol: ${record.symbol}`);
      continue;
    }
...
function checkAndSendAlerts(records) {
  ...
  for (const record of records) {
    ...
    try {
      const alertsResponse = UrlFetchApp.fetch(`${CONFIG.SUPABASE_URL}/rest/v1/barchart_alerts?symbol=eq.${record.symbol}&is_active=eq.true`, {
        method: 'GET',
        headers: {
          'apikey': CONFIG.SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${CONFIG.SUPABASE_ANON_KEY}`,
